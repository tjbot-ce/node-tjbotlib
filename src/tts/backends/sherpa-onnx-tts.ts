...
export class SherpaONNXTTSEngine extends TTSEngine {
    ...
    private modelInfo?: TTSModelMetadata;
    ...
    async initialize(): Promise<void> {
        const config = this.config as TTSBackendLocalConfig;
        ...
        this.modelInfo = await this.registry.loadModel<TTSModelMetadata>(modelName);
     ...
     */
    private async setupSynthesizer(): Promise<void> {
        if (!this.modelInfo) {
            throw new TJBotError('Model info not set. Ensure initialize() was called.');
        }
     ...
     */
    private resolveVitsDataDir(modelDir: string): string {
     ...
     */
    private createOfflineTTS(modelFile: string, vitsDataDir: string): OfflineTts {
        if (!sherpa) {
            throw new TJBotError('Sherpa-ONNX module not loaded. Ensure initialize() was called.');
        }
