...
export function resolveCredentialsPath(filename: string, providedPath?: string): string {
    ...
    throw new TJBotError(
        `Credentials file ${filename} not found. Place credentials at: ./${filename} or ~/.tjbot/${filename}`
    );
}
 ...
 */
function parseEnvCredentialsFile(credentialsPath: string): Record<string, string> {
...
export function loadIBMWatsonCloudCredentials(providedPath?: string): void {
    ...
    try {
        ...
        throw new TJBotError(`Failed to load IBM Watson credentials from ${credentialsPath}`, { cause: err as Error });
    }
}
